...
class ChewieState extends State<Chewie> {
  ...
  Future<void> listener(VideoEventType event) async {
    if (event == VideoEventType.startWindowFullscreen ||
        event == VideoEventType.exitWindowFullscreen) {
...
class ChewieController extends ChangeNotifier {
  ...
  Future<dynamic> _initialize() async {
    ...
    if ((autoInitialize || autoPlay) &&
        !videoPlayerController.value.onVideoPlayerInitialized) {
...
class FullScreenPage extends StatefulWidget {
  const FullScreenPage(
      {super.key, required this.controllerProvider, required this.controller});
          ...
          if (rotation != newRotation && widget.controller.isRotateWithSystem) {
            ...
            Timer(const Duration(milliseconds: 100), () {
              widget.controller.videoPlayerController
                  .postEvent(VideoEventType.onRotateChanged);
            });
          }
        }),
      );
